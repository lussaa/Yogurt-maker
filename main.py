...
def makeSomeYogurt(p100):
    ...
    #first phase
    while durationSecs(start_time) < 4500: # 1 hour 15mins
        try:
            humidity, temperature = Adafruit_DHT.read_retry(sensor, gpio)
            if humidity is not None and temperature is not None:
                logger.info('Current temperature: %s' % temperature)
                light_state = controlTemperature(int(temperature), light_state, p100, TEMP_FIRST_STAGE)
            else:
    ...
    #second phase
    while durationSecs(start_time) < 14400: #4 hours
...
def controlTemperature(current_temp, light_state, p100, ideal_temp):
    if (current_temp > ideal_temp ):
        if (light_state == light_stateON):
            ...
            logger.info("Current temp bigger or equal to %s so light goes off." % ideal_temp)
    if (current_temp < ideal_temp):
        if (light_state != light_stateON):
